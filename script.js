...
function changeSection(num) {
    ...
    // Show tasks section in section view
...
function showGroupSchedule(group) {
    ...
    // Hide tasks section in group view
...
function parseCSVTasks(csv) {
    ...
    // نبدأ من الصف 2 (index 2) عشان نتخطى الـ headers
    // Row 0: English headers, Row 1: Arabic headers, Row 2+: Data
...
function renderTasks() {
    ...
    container.innerHTML = filtered.map(task => {
        ...
        if (daysLeft !== null) {
            if (daysLeft < 0) {
                countdownHtml = `<div class="task-countdown overdue"><i class="fas fa-exclamation-circle"></i> ${Math.abs(daysLeft)} days ago</div>`;
            } else if (daysLeft === 0) {
                countdownHtml = `<div class="task-countdown today"><i class="fas fa-clock"></i> Due Today!</div>`;
            } else if (daysLeft === 1) {
                countdownHtml = `<div class="task-countdown tomorrow"><i class="fas fa-hourglass-half"></i> 1 day left</div>`;
            } else {
                countdownHtml = `<div class="task-countdown"><i class="fas fa-hourglass-start"></i> ${daysLeft} days left</div>`;
            }
        }
...
function parseDateSafe(dateString) {
    ...
    // جرب قراءة مباشرة أولاً (YYYY-MM-DD)
    let date = new Date(str);
    if (!isNaN(date.getTime())) return date;
    
    // لو فيه / حاول DD/MM/YYYY
    if (str.includes('/')) {
